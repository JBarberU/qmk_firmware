...
void init_styles(void) {
    ...
    lv_style_set_text_color(&style_splash_text, lv_palette_main(LV_PALETTE_PURPLE));
}
...
void init_screen_home(void) {
    ...
    lv_obj_t *label_splash = lv_label_create(screen_home);
    lv_label_set_text(label_splash, "Heimdall");
    lv_obj_add_style(label_splash, &style_splash_text, 0);
...
bool display_init_kb(void) {
    ...
    painter_device_t display = qp_st7789_make_spi_device(240, 300, LCD_CS_PIN, LCD_DC_PIN, LCD_RST_PIN, SPI_DIVISOR, SPI_MODE);
